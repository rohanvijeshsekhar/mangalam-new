...
function allTickets($searchData = '', $filter = 0)
{
    if (isset($_GET['val'])) {
        $searchData = $_GET['val'];
    }
    ...
    $fetchTickets = $obj->selectData("ticket_id,title,featured,destination_id,slug_url,card_image,validity,display_amount", "tickets", "where status != 0 $where1 order by featured desc,ticket_id desc");
    if ($fetchTickets && safe_mysqli_num_rows($fetchTickets) > 0) {
        $m = 0;
        while ($data_row = safe_mysqli_fetch_array($fetchTickets)) {
            $ticketId       = $data_row['ticket_id'];
